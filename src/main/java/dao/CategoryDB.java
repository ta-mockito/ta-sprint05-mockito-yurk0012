...
public class CategoryDB implements CategoryDAO, AutoCloseable {
    ...
    private static final String SQL_SELECT_CHILDREN_BY_CATEGORY = "SELECT DISTINCT ch.id, ch.first_name, ch.last_name, ch.birth_date " +
            "FROM child ch " +
            "JOIN club_child cc ON ch.id = cc.child_id " +
            "JOIN club cl ON cc.club_id = cl.id " +
        ...
        try (PreparedStatement statement = conn.prepareStatement(SQL_SELECT_CHILDREN_BY_CATEGORY)) {
            ...
            try (ResultSet rs = statement.executeQuery()) {
                while (rs.next()) {
                    ...
                    LocalDate date = birthDate != null ? birthDate.toLocalDate() : null;
